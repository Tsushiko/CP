...
\textbf{Grupo} nr. & 49
\\\hline
a95869 & André Neves da Costa
\\
a97666 & Tiago Emanuel Lemos Teixeira 
\\
a97509 & Bruno Miguel Fernandes Araújo
\\
a96156 & Filipe José Silva Castro
\end{tabular}
\end{center}
...
|(Nesta resolução assumimos que aux2 é uma nova versão de aux)|
...
Partindo das definições de |q|,|r| e |c| e aplicando
a |regra de algibeira| obtemos que
\begin{code}
aux2 d = for ((loop2 d)) ((0,(0,d))) where
    loop2 d (q,(r,c)) = (q + (if c == 0 then 1 else 0),(if c == 0 then 0 else 1 + r, if c == 0 then d else c - 1))
\end{code}

A partir da nova definição de aux, separando os casos em que |c == 0| e |c /= 0| (removendo assim os if then else)

|(É usado c+1 para c/= 0 pois como c devolve um numero natural, c+1 será sempre maior que 0)|

\begin{code}
aux2 d = for ((loop2 d)) ((0,(0,d))) where
    loop2 d (q,(r,0)) = (q + 1,(0,d))
    loop2 d (q,(r,c+1)) = (q,(1 + r,c))
\end{code}

E assim, obtemos, a partir de aux, uma definição equivalente a loop.

\subsection*{Problema 2}

Para resolver este problema nós primeiramente definimos as funções:

\begin{code}

a :: Ord c => LTree c -> c
a (Leaf x) = x
a (Fork(l1,l2)) = max (b(l1)) (b(l2))


b :: Ord c => LTree c -> c
b (Leaf x) = x
b (Fork(l1,l2)) = min (a(l1)) (a(l2))


both1 :: Ord d => LTree d -> (d,d)
both1 t = (a(t),b(t))

\end{code}

Partimos depois destas para definir a sua versão point-free
de forma a usar mais para a frente as leis da recursividade mútua:
...
\start
        |lcbr(
          ...
          alice (Fork (l1,l2)) = max (bob(l1)) (bob(l2))
     )|
%
\just\equiv{ (84), (71) }
%
        |lcbr(
          ...
          (alice . Fork) (l1,l2) = uncurry max (bob(l1), bob(l2))
     )|
%
\just\equiv{ (77), (72) }
%
        |lcbr(
          ...
          (alice . Fork) (l1,l2) = (uncurry max . (bob >< bob)) (l1,l2)
     )|
%
\just\equiv{ (71) }
%
...
%
\just\equiv{ (27), (20), inLTree = [Leaf, Fork]}
%
     |alice . in = either id (uncurry max . (bob >< bob))|
%
\just\equiv{ (7), (14), (22) }
%
...
Fazendo o mesmo para a função bob obtemos:
...
\start
        |lcbr(
          ...
          bob . in = either id (uncurry min . (p1 >< p1)) . (id + ((split alice bob) >< (split alice bob)))
     )|
%
\just\equiv{ F f = id + f², (52)}
%
...
alice = either id (uncurry max . (bob >< bob)) . outLTree



bob :: Ord c => LTree c -> c
bob = either id (uncurry min . (alice >< alice)) . outLTree
...
&
    |A + (LTree A)|^2
           \ar@@/^2pc/[l]^-{|inLTree|}
           \ar[d]^{|id + split alice bob|^2}
\\
     |(Nat0)|^2
&
     |A + (Nat0)|^2
           \ar[l]^-{|either (either id (uncurry max . (p1 >< p1))) (either id (uncurry min . (p2 >< p2)))|}
}
\end{eqnarray*}

\subsection*{Problema 3}

Diagrama geral de |LTree3|
...
\xymatrix@@C=2cm@@R=2cm{
    |LTree3 A|
           ...
           \ar@@/^1pc/[r]^-{|outLTree|}
&
    |A + LTree3 A >< (LTree3 A)|^2
           \ar@@/^1pc/[l]^-{|inLTree|}
           \ar[d]^{|id + cata f >< cata f|^2}
\\
     |B|
&
     |A + B >< B|^2
...
Diagrama do hilomorfismo |sierpinski|
...
&
    |A + B|^2|>< B|
           \ar[d]^-{|id + geraSierp|^2| >< geraSierp|}
\\
    |LTree3 A|
           \ar@@/^1pc/[r]^-{|outLTree|}
           \ar[d]_-{|folhasSierp|}
&
    |A + (LTree3 A)|^2|>< (LTree3 A)|
           \ar@@/^1pc/[l]^-{|inLTree|}
           \ar[d]^-{|id + folhasSierp|^2|>< folhasSierp|}
\\
    |A|^*
&
    |1 + A >< A|
...
Como folhasSierp transforma uma LTree3 numa lista de triangulos, g1 seria um either que
aplica singl aos triangulos |(g1 Tri = [Tri])| e concatena-os a todos.

\begin{code}
g1 = either singl (conc . (split (conc . p1) p2))
\end{code}

GeraSierp transforma (((x,y),Side),n) em LTree3.
Quando recebe |n == 0|, g2 devolve apenas o triangulo 
((x,y),Side) e quando recebe |n /= 0| cria a
LTree3 com altura igual a n e, por cada 1 de altura, 
diminui o tamanho dos triangulos que estarão nas folhas por metade.

\begin{code}
g2 (t,0) = i1 t
g2 (((x,y),s),n+1) = i2((t1,t2),t3) where
     t1 = (((x,y),(s`div`2)),n)
     t2 = (((x,y+(s`div`2)),(s`div`2)),n)
